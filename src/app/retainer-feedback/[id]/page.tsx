...
const step4Fields = [
  ...
  options: [
      ...
      { value: "Good", label: "Good" },
      { value: "Average", label: "Average" },
      { value: "Poor", label: "Poor" },          // Changed from "Very Slow" to fit "creative work"
      { value: "Very Poor", label: "Very Poor" },// Changed from "Slow" to fit "creative work"
    ],
  },
  {
    ...
    label:
      "15. Were there any deliverables that pleasantly surprised you? If so, we would love to know which ones and what made them stand out for you. ",
    fieldType: "text",
  },
  {
...
export default function RetainerFeedbackPage() {
  ...
  const handleSubmit = async () => {
    const payload = {
      ...
      other_service_description: formData.services_provided?.list?.includes("other") ? formData.services_provided?.other_service_description : null,
  ...
  const renderStepFields = (fields: any[]) => (
    <div className="px-4 max-w-2xl w-full pb-3 mx-auto md:px-0">
      ...
      {fields.map((field) => (
        <DynamicField key={field.id} field={field} value={formData[field.name]} onChange={(val) => updateFormData({ [field.name]: val })} />
      ))}
      <div className="flex gap-4 mt-8">
        {step > 1 && (
          <Button onClick={handleBack} className="!bg-white !text-black border border-gray-300 !text-lg w-full max-w-2xl mx-auto">
            Back
          </Button>
        )}
      ...
      {step === STEPS_COUNT && (
        <div className="mt-8 text-center text-gray-600 text-sm max-w-2xl mx-auto">
          <p>
            This feedback form is used to review and evaluate individual employee performance based on different criteria. It helps the organization
            understand performance levels and areas for improvement.
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div className="relative min-h-screen !bg-[#FFFBFB] flex flex-col py-10 justify-center overflow-hidden">
      ...
      {step > 1 && <StepIndicator step={step} stepStructure={stepStructure} getStepProgress={getStepProgress} handleStepClick={handleStepClick} />}
